...
### Triballs
The field is set up with 12 neutral Triballs that are placed in the same position at the beginning of each match. During the match, each alliance has 22 neutral Triballs that can be matchload into the Match Load Zones or gently matchload onto their robot that is contacting the Match Load Zone. The Triballs can be found in the green neutral color, or  red or blue designating each alliance. 
...
During the match, robots can move Triballs into either their alliance's Goal, or in their alliance's offensive zone. Each robot is only able to be in possession of 1 triball at a time.
